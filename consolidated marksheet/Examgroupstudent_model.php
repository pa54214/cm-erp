...

{
    ...
    public function searchExamStudentByExam($exam_id)
    ...
    public function searchExamStudentByExamNew($exam_id, $student_id, $session_id)

    {

        $sql = "SELECT exam_group_class_batch_exam_students.id as `exam_group_class_batch_exam_student_id`, exam_group_class_batch_exam_students.rank, exam_group_class_batch_exam_students.roll_no as `exam_roll_no`, exam_group_class_batch_exam_students.teacher_remark, students.admission_no, students.id as `student_id`, students.roll_no, students.admission_date, students.firstname, students.middlename, students.lastname, students.image, students.mobileno, students.email, students.state, students.city, students.pincode, students.religion, students.dob, students.current_address, students.permanent_address, students.category_id, IFNULL(categories.category, '') as `category`, students.adhar_no, students.samagra_id, students.bank_account_no, students.bank_name, students.ifsc_code, students.guardian_name, students.guardian_relation, students.guardian_phone, students.guardian_email, `classes`.`class`, students.guardian_address, students.is_active, `students`.`father_name`, `students`.`gender`, `students`.`app_key`, `students`.`parent_app_key`, exam_group_class_batch_exam_students.rank, sections.section as student_section
            FROM `exam_group_class_batch_exam_students`
            INNER JOIN student_session ON student_session.id = exam_group_class_batch_exam_students.student_session_id
            INNER JOIN students ON students.id = student_session.student_id
            INNER JOIN `classes` ON `student_session`.`class_id` = `classes`.`id`
            INNER JOIN `sections` ON `student_session`.`section_id` = `sections`.`id`
            LEFT JOIN `categories` ON `students`.`category_id` = `categories`.`id`
            WHERE exam_group_class_batch_exam_id = " . $this->db->escape($exam_id) . " AND students.is_active = 'yes' AND students.id = " . $this->db->escape($student_id) . " AND student_session.session_id = " . $this->db->escape($session_id) . "
            ORDER BY exam_group_class_batch_exam_students.rank ASC
            LIMIT 1";
    ...
    public function searchExamStudentsNew($exam_group_id, $exam_id, $class_id, $session_id)
